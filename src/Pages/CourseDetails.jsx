...
function CourseDetails() {
  ...
  const { courseId } = useParams(); // ✅ Destructure correctly

  // Get token and user from Redux or localStorage
  ...
  const handleBuyCourse = () => {
    ...
    // If logged in, proceed with buying the course
    if (courseId) {
  ...
  // State to hold the extracted course details object
    ...
    const fetchCourseDetails = async () => {
      try {
        ...
        if (response.success) {
          ...
          console.log("Fetched course details response:", response);
          console.log("Extracted course details object:", response.data);
        } else {
  ...
  useEffect(() => {
    if (courseDetails?.ratingAndReviews) {
      ...
      console.log("Average Review Count:", count);
    }
  }, [courseDetails]);
  ...
  useEffect(() => {
    ...
    console.log("Total Number of Lectures:", totalLectures);
  }, [courseDetails]);


  // Consolidated loading and not found checks
  ...
  // Destructure directly from courseDetails
  const {
    courseName,
    courseDescription,
    // thumbnail, // Not used in this render block, but good to have if you add an image
    courseContent,
    ratingAndReviews,
    // whatYouWillLearn, // Not used in this specific display, but good to have
    price,
    instructor,
    studentsEnrolled,
       ...
       <div className=' flex gap-16 mt-2 '>
         <div className='flex ' >
           ...
           <div className=' mx-5' onClick={()=> setIsActive([])}>
                <button> collaps all section</button>
           </div>
